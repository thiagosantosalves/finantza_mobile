...
const CategoryDebitEdit = ({ navigation, route, ...rest }) => {
    ...
    useEffect(() => {
        const url = listIconDp.filter(item => item.id === route.params.category.id_icon)
        setUrlIcon(url[0]);
    }, []);

    const handlerIcon = (id) => {  
        const iconInfo = listIconDp.filter(item => item.id === id);
    ...
    const handleCreate = async () => {
        ...
        if(name && urlIcon && color_hex) {
            ...
            await api.put(`dpcategory/${route.params.category.id}`, category);

            navigation.navigate("categoryDebit");
...
export default CategoryDebitEdit;